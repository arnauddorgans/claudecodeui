...
A PTY is keyed by project path and session id, and outlives the socket that opened it: every socket that sends the same `init` joins its audience and gets the same output (the newest used to take the stream away from the others), with the last 5000 chunks replayed on attach. Under `auto` a PTY nobody watches is killed after `PTY_SESSION_TIMEOUT_MS` (30 min, `0` for never); under `manual` it stays until its shell exits or `chat.close` on the session ends it. A PTY resuming a provider session is registered as that session's `terminal` process.
